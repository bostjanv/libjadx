...
| Unknown native JSON field preservation | SUPPORTED at project-root and `codeData` object level | `NativeProjectDocumentTest` proves unknown project, tab and top-level code-data fields survive explicit code-data writes. Unknown members nested inside an edited rename/comment entry are not covered. `ProjectData` model serialization alone drops unknown fields (`NativeProjectModelProbeTest`). |
