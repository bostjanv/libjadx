...
| Java class source | SUPPORTED for owned JAR/native fixtures; PARTIAL across all Jadx inputs | `JadxSourceProbeTest` and `DecompileEndpointsTest` compare the returned string with pinned `ICodeInfo.getCodeStr()`. Source ownership is obtained from `getTopParentClass()`, including the owned inlined anonymous class. Missing output remains `UNAVAILABLE`. |
